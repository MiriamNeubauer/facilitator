...
27. AMASON S3 Image-Upload:
	...
	f) specify <2.0 for aws-sdk-Gem because later versions are unstable
	g) create /initializers/paperclip.rb:
	# We are actually setting this to 's3_domain_url', 
	# so it's not a placeholder for something else. 
	Paperclip::Attachment.default_options[:url] = ':s3_domain_url'
	Paperclip::Attachment.default_options[:path] = '/:class/:attachment/:id_partition/:style/:filename'
	Why? To display images because image path is incorect, 2 variables need to switch place in the url, and paperclip.rb takes care of that.


28. NEXT-LEVEL: STYLING WITH JQUERY, INTERNATIUONALIZATION, COMMENTS
	a) changed root-route from pages#home to pins#index, and added singup-and-loginbutton to bottom of it via creating the partial pages/_signup.html.erb by just renaming pages/home.html.erb cause we don;t need that anymore anyway
	b) created
	
29. accidentally committed my secret AWS keys to Github:
...
- pitv ENV variables off github!! accidentally pushed them once at 10:30 pm on June 18th
